...
const DeckCard = ({specDiv, specChip}) => {
  ...
  return (
    <div className="gap-1.5 xl:gap-2 w-full flex flex-col xl:flex-row min-w-[280px]">
      <div className="hidden md:block flex-1 bg-cover bg-center rounded-lg bg-[url('/pixel.gif')] border border-neutral-800"></div>
      <div className="grid grid-cols-2 md:grid-cols-1 gap-1.5 xl:gap-2 xl:w-[320px]">
        <div className={`${specDiv} gap-y-2`}>
          <div className="text-[12px] md:text-[14px]">{language === "en" ? "Created with :" : "Dibuat dengan :"}</div>
        ...
        <div className={`${specDiv} gap-y-1.5`}>
          <div className="text-[12px] md:text-[14px]">{language === "en" ? "My Time Zone :" : "Zona Waktu Saya :"}</div>
          <div className="flex">
            <Clock />
          </div>
        </div>
